...
class Errors:
    ...
    @staticmethod
    def undefined_transition(trans):
        stream = open(Errors.output, "w")
...
def checkStates(states, inits, fins, trans):
    ...
    for i in range(0, len(trans), 3):
        if trans[i] not in states:
            Errors.undefined_state(trans[i])

    #checking states in fins
    for st in fins:
        if st not in states:
            Errors.undefined_state(st)
